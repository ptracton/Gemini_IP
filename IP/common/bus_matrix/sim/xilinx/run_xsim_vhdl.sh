...
# Description: Xilinx xsim orchestration script for Bus Matrix IP (VHDL).
#
# How it operates:
...
for test in "${TESTS[@]}"; do
    ...
    xvhdl -2008 \
        ...
        $RTL_DIR/bus_matrix_regs.vhd \
        $RTL_DIR/bus_matrix_core.vhd \
        $RTL_DIR/bus_matrix_apb.vhd \
        $RTL_DIR/bus_matrix_axi.vhd \
        $RTL_DIR/bus_matrix_wb.vhd

    echo "--- Compiling Testbench ---"
    xvhdl -2008 $TB_DIR/tb_bus_matrix_$test.vhd

    echo "--- Elaborating ---"
    xelab -debug typical tb_bus_matrix_$test -s snapshot_tb_bus_matrix_${test}_vhd -cc_type bbc

    echo "--- Simulating ---"
    xsim snapshot_tb_bus_matrix_${test}_vhd -runall -log xsim_${test}.log

    # Archive Coverage
    COV_REPO_DIR="$IP_DIR/coverage_repo"
    mkdir -p $COV_REPO_DIR
    if [ -d "xsim.codeCov/snapshot_tb_bus_matrix_${test}_vhd" ]; then
         rm -rf $COV_REPO_DIR/xsim_vhd_$test
         cp -r xsim.codeCov/snapshot_tb_bus_matrix_${test}_vhd $COV_REPO_DIR/xsim_vhd_$test
    fi
